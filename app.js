const data = window.AI_LIBRARY_DATA;
...
function buildStats() {
  const stats = [
    {
      label: "总条目数",
      value: data.meta.totalRecords,
      note: "工作簿跨 7 个主题表整合",
    },
    {
      label: "问题条目",
      value: data.meta.issueRecords,
      note: "需要补链、补日期或修正字段",
    },
    {
      label: "覆盖主题",
      value: data.topicBreakdown.filter((item) => item.count > 0).length,
      note: "战略、机构、规范、评估、标准、立法、国际治理",
    },
    {
      label: "国际/地区节点",
      value: data.countryBreakdown.length,
      note: "按国家/地区字段聚合后的重点节点",
    },
  ];
...
function buildTopicBars() {
  ...
  topicBars.innerHTML = data.topicBreakdown
    .map(
      (item) => `
        <div class="bar-row">
          <div class="bar-meta">
...
function buildSpotlight() {
  spotlightList.innerHTML = data.spotlight
    .map(
      (item) => `
...
function buildTimeline() {
  ...
  timelineChart.innerHTML = data.timeline
    .map(
      (item) => `
...
function buildCountryCloud() {
  countryCloud.innerHTML = data.countryBreakdown
    .map(
      (item) => `
...
function linkMarkup(record) {
  const links = [];
  if (record["主参考链接"]) {
    links.push(`<a href="${record["主参考链接"]}" target="_blank" rel="noreferrer">主链接</a>`);
  }
  if (record["补充链接"]) {
    links.push(`<a href="${record["补充链接"]}" target="_blank" rel="noreferrer">补充链接</a>`);
  }
  return links.join("");
}
...
function buildIssues() {
  issueList.innerHTML = data.issueRecords
    .map(
      (record) => `
...
function buildRecent() {
  recentList.innerHTML = data.recentRecords
    .map(
      (record) => `
